...
const reset = () => {
  ...
  document.getElementById("morseJapanese").innerHTML = `<span></span>${morseText.replace(/\|/g, '　')}`;
  num ++;
}
...
const colorChange = (text, file) => {
  ...
  if (file === "morseJapanese") {
    const part1 = text[0].replace(/\|/g, '　');
    const part2 = text[1].replace(/\|/g, '　');
